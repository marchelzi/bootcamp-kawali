from admin import cekUser,current_users, add_users, show_users, delete_users,transactions, show_transactions, accounts 
...
def choice_menu(current_users):
    if current_users['role'] == 'admin':
        while True:
            ...
            if choice == 1:
                add_users(cekUser, current_users)
                # print(users)
            elif choice == 2:
                delete_users(cekUser, accounts, transactions)
                pass
            elif choice == 3:
                show_users(cekUser)
                pass
